...
pacman::p_load(tidyverse, readxl,
               bipartite, car, cluster, Hmisc, corrplot, vegan, ade4,
               ggpubr, viridis, RColorBrewer, cowplot, svglite)
# select <- dplyr::select
...
plant_traits <- read_excel("input/cheaters_visitation_and_trait_data.xlsx",  
                           sheet = "plant_traits") %>% 
...
robber_orders <- cheater_data %>%
  ...
  filter(behavior == "robbing") %>% 
  
  select(functional_group) %>% unique()

# Subset of robbed plants

robbed_plants <- cheater_data %>% filter(behavior == "robbing") %>% 
...
robbing_rda <- cheater_data %>% group_by(spcode, functional_group) %>%
  ...
  mutate(robfreq = ifelse(behavior=="robbing", freq_fm_per_species, 0)) %>%
...
envrob <- robbed_plant_traits[,c(2:11)]
names(envrob)
...
thief_orders <- cheater_data %>%
  ...
  filter(behavior == "thieving") %>% 
  
  select(functional_group) %>% unique()

# Subset of thieved plants

thieved_plants <- cheater_data %>% filter(behavior == "thieving") %>% 
...
thieving_rda <- cheater_data %>% group_by(spcode, functional_group) %>%
  ...
  mutate(thieffreq = ifelse(behavior=="thieving", freq_fm_per_species, 0)) %>%
